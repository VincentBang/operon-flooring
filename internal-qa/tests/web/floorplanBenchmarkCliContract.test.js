...
(function testBenchmarkCliWritesLocalArtifacts() {
  ...
  assert.ok(result.stdout.includes("\"hybrid_selector_spike\""), "CLI JSON should include hybrid selector spike summary.");
  ...
  assert.equal(json.hybrid_selector_spike.passed_contract_count, json.hybrid_selector_spike.item_count);
  assert.equal(json.hybrid_selector_spike.measured_warning_count, 0);
  ...
  assert.ok(markdown.includes("Hybrid Selector Spike Candidates"));
