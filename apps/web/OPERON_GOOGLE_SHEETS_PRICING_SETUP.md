...
- if a supplier colour page exposes multiple usable gallery images, keep a few local images in that colour popup so customers can browse more than one photo
...
- if a colour page only provides one usable image, extend that colour popup with a few gallery images inherited from the same range so the customer still sees a fuller preview
...
- range-level technical content should include plank, board, or panel size whenever the supplier provides it
...
- all range cards on `products.html` should use the same colour-preview pattern:
  ...
  - show a visible `Range details` / product information action on the card when range metadata exists
